...
本文根据当前仓库实现整理，更新于 2026-09-16。安装和操作步骤见[部署与运行文档](付费报告部署与运行.md)。

快速定位：[总体架构](#2-总体架构) · [代码分层](#3-代码分层) · [购买流程](#4-一次购买的完整流程) · [完整用户时序](#45-完整用户时序与前端调用) · [数据模型](#6-数据模型) · [报告保存](#8-报告持久保存) · [接口](#9-对外接口)。
...
1. 首页提交 `ticker`（股票）、`language`（报告语言）和 `recaptcha_token`（一次性人机验证令牌），同时携带 `Idempotency-Key` 请求标识。
2. 后端先按 IP 限流，检查销售开关和必要配置，再向 Google 验证 reCAPTCHA v2 令牌及域名；验证失败或不可用时拒绝下单。随后检查队列容量、股票代码格式及标的资格。无法确认股票信息时，不进入收款步骤。
...
同一浏览器会话的相同股票与语言会复用请求标识；验证码令牌不参与幂等标识计算、不存入会话或订单，每次重试需重新验证。若创建收银台超时，远端可能已经创建成功，本地标记 `UNKNOWN`；重试不会盲目再次创建远端收银台，需要人工核对。
...
下面以销售已开放、reCAPTCHA 已配置、首次下单并成功生成报告为例。浏览器栏包含用户在网页中的操作；数据库栏同时代表 SQLite 订单和任务队列，后台栏合并展示分析线程与交付协调线程。两个 `par` 分支可以同时进行：支付回调与浏览器返回网站的先后顺序不固定。
...
sequenceDiagram
    ...
    B->>A: GET / 打开首页
    A-->>B: HTML，语言选项，公开 site key
    B->>A: GET /static/app.js 与 style.css
    B->>G: 加载 api.js，完成复选框验证
    G-->>B: JS 回调返回一次性 token
    Note over B: 选择股票和语言，点击生成报告<br/>同一组输入复用 Idempotency-Key
    B->>A: POST /api/orders<br/>ticker、language、recaptcha_token<br/>请求头 Idempotency-Key
    A->>A: 按 IP 限流，校验来源、输入和销售配置
    A->>G: POST /recaptcha/api/siteverify<br/>服务端 secret 与 token
    G-->>A: success 与 hostname
    A->>A: 验证结果和域名，检查幂等与队列容量<br/>校验股票，生成服务端参数快照
    A->>D: 保存 PENDING_PAYMENT 订单及访问令牌
    A->>C: 读取并校验产品，POST /v1/checkouts
...
| 时机 | 请求 | 前端处理 |
| --- | --- | --- |
| 打开首页 | `GET /`、`GET /static/app.js`、`GET /static/style.css` | 展示表单；没有销售资格时按钮关闭；有资格时加载 Google 验证组件 |
...
{"ticker":"AAPL","language":"zh-CN","recaptcha_token":"一次性验证码令牌"}
```
...
异常路径也由当前前端处理：下单遇到验证码无效或过期、网络或服务错误，会显示错误并重置验证码，重新验证后沿用相同股票与语言对应的幂等标识。下单返回 429 时按钮按 `Retry-After` 暂停重试，当前后端返回 60 秒；状态查询遇到 429 也延长轮询间隔，其他查询错误默认 5 秒后重试。到达状态页时已经处于终态，则无需启动轮询。
...
| `POST /api/orders` | JSON 只接受 `ticker`、`language`、`recaptcha_token`；需要 `Idempotency-Key`；默认必须通过人机验证，返回收银台地址及状态页地址 |
...
默认开启 Google reCAPTCHA v2 复选框，密钥不完整时关闭新购买。后端以固定 Google HTTPS 接口校验令牌，要求成功且 `hostname` 匹配公开域名，网络或配置异常不会绕过验证；只有测试模式允许明确关闭。支付回调和已购报告无需人机验证。购买页 CSP 只放行验证码所需 Google/gstatic 来源，私密报告页不加载验证码脚本。
