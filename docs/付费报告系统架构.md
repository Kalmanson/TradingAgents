...
图中的供应商分支由 `data_vendors` 和优先级更高的 `tool_vendors` 决定。购买资格校验使用付费服务自身配置，后台分析使用订单保存的配置快照；选择 FMP 不会隐式回退到 Yahoo。宏观、预测市场和社交数据保留各自的数据路径，未在图中展开。
...
| `fundamental_data`：基本面与三张财报 | `fmp` | `yfinance` |
